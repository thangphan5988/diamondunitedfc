...
function setBench(id,bench,editTeam){
  ...
  root.innerHTML = bench.map(p => {
    ...
    return `<div class="${cls}"${attrs}><span class="benchRating">${p.rating || 5}</span><img src="${escapeAttr(p.avatar)}">${escapeHtml(p.name)} · ${p.main}</div>`;
  }).join("");
